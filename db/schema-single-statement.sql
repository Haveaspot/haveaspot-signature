-- =============================================================================
-- HAS-Signature schema — single-statement version.
--
-- Identical to db/schema.sql, wrapped in a DO block so it can be run in one go
-- from Neon's Query console, which sends input as a single prepared statement
-- and rejects multiple semicolon-separated commands.
--
-- Turn OFF the console's "Read-only" toggle before running.
